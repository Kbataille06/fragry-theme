...
for p in products:
    ...
    row["Handle"] = p["handle"]
    row["Title"] = p["title"]
    row["Body (HTML)"] = template_html(p["desc"], p["top"], p["heart"], p["base"], p["size"], p["brand"])
    row["Vendor"] = "K&M parfumerie"
    row["Product Category"] = "Health & Beauty > Personal Care > Cosmetics > Perfumes & Colognes > Eaux de Parfum"
    row["Published"] = "true"
    row["Option1 Name"] = "Title"
    row["Option1 Value"] = "Default Title"
    row["Variant Grams"] = "500.0"
    row["Variant Inventory Tracker"] = "shopify"
    row["Variant Inventory Qty"] = "5"
    row["Variant Inventory Policy"] = "deny"
    row["Variant Fulfillment Service"] = "manual"
    row["Variant Price"] = p["price"]
    row["Variant Compare At Price"] = p["compare_price"]
    row["Variant Requires Shipping"] = "true"
    row["Variant Taxable"] = "true"
    row["Gift Card"] = "false"
    row["Occasion (product.metafields.shopify.occasion)"] = p["category"]
    row["Saison (product.metafields.shopify.season)"] = p["season"]
    row["Variant Weight Unit"] = "kg"
    row["Status"] = "active"
    
    # ensure correct ordering
    ordered_row = [row[col] for col in header]
    rows.append(ordered_row)

import csv
with open("fragry_products_flawless.csv", "w", encoding="utf-8-sig", newline='') as f:
    writer = csv.writer(f)
