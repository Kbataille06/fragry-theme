...
for (const p of products) {
    const htmlBody = templateHTML(p.title, p.desc, p.top, p.heart, p.base, p.size, p.brand);
    const row = [
        p.handle,                           // Handle
        p.title,                            // Title
        htmlBody,                           // Body (HTML)
        "K&M parfumerie",                   // Vendor
        "Health & Beauty > Personal Care > Cosmetics > Perfumes & Colognes > Eaux de Parfum", // Product Category
        "",                                 // Type
        "",                                 // Tags
        "true",                             // Published
        "Title",                            // Option1 Name
        ...
        "5",                                // Variant Inventory Qty
        "deny",                             // Variant Inventory Policy
        "manual",                           // Variant Fulfillment Service
        p.price,                            // Variant Price
        p.compare_price,                    // Variant Compare At Price
        "true",                             // Variant Requires Shipping
        "true",                             // Variant Taxable
        "", "", "", "", "",
        "",                                 // Image Src -> BLANK TO PREVENT SHOPIFY IMPORT FAILURE ON 404
        "", "", "false", "", "",
        p.category,                         // Occasion
        p.season,                           // Saison
        "",                                 // Variant Image
...
// Write file with UTF-8 BOM explicitly
fs.writeFileSync('fragry_products_final.csv', String.fromCharCode(0xFEFF) + rows.join('\r\n'), 'utf8');
console.log('CSV generated as fragry_products_final.csv');
